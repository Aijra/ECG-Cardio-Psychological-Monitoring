...
% Bandpass filter (remove baseline + high freq noise)
[b,a] = butter(3,[0.5 40]/(Fs/2),'bandpass');
ecg = filtfilt(b,a,ecg);

%% ================= WAVELET DECOMPOSITION =================
% Using Daubechies wavelet (best for ECG)
level = 5;
wname = 'db4';

[C,L] = wavedec(ecg, level, wname);

% Reconstruct detail coefficients (QRS band ~5–25 Hz)
D3 = wrcoef('d', C, L, wname, 3);
D4 = wrcoef('d', C, L, wname, 4);

% Combine relevant bands
qrs_band = D3 + D4;

%% ================= ENERGY ENHANCEMENT =================
% Square to enhance peaks
energy = qrs_band.^2;

% Smooth using moving average (~150 ms)
win = round(0.15 * Fs);
energy_smooth = movmean(energy, win);

% Normalize
energy_smooth = energy_smooth / max(energy_smooth + eps);

%% ================= ADAPTIVE THRESHOLD =================
THR_SIG = mean(energy_smooth) + 0.5*std(energy_smooth);
THR_NOISE = 0.5 * THR_SIG;

[peaks, locs] = findpeaks(energy_smooth, ...
...
for i = 1:length(peaks)
    ...
    if pk > THR_SIG
        ...
        if ~isempty(detected_peaks)
            ...
            prev = detected_peaks(end);
            RR = (loc - prev)/Fs;

            if RR < 0.36

                l1 = max(1, prev-10); r1 = min(length(qrs_band), prev+10);
                l2 = max(1, loc-10);  r2 = min(length(qrs_band), loc+10);

                slope1 = max(abs(diff(qrs_band(l1:r1))));
                slope2 = max(abs(diff(qrs_band(l2:r2))));

                mean_RR = mean(RR_list + eps);

                % SAME EXTRA CONDITION (your improvement)
        ...
        %% --- ACCEPT PEAK ---
        detected_peaks(end+1) = loc;
        last_peak = loc;

        THR_SIG = 0.125*pk + 0.875*THR_SIG;

        %% --- RR UPDATE ---
        if length(detected_peaks) > 1
            RR = (detected_peaks(end) - detected_peaks(end-1))/Fs;
            RR_list(end+1) = RR;

            if length(RR_list) >= 8

                RR_avg1 = mean(RR_list(end-7:end));

                RR_valid = RR_list( ...
                    RR_list > 0.92*RR_avg1 & RR_list < 1.16*RR_avg1);

                if length(RR_valid) >= 8
                    RR_avg2 = mean(RR_valid(end-7:end));
                end
            end
        end
        ...
        %% --- NOISE UPDATE ---
        THR_NOISE = 0.125*pk + 0.875*THR_NOISE;
    end

    %% --- THRESHOLD UPDATE ---
    THR_SIG = THR_NOISE + 0.25*(THR_SIG - THR_NOISE);

    %% --- IRREGULAR RHYTHM ADJUSTMENT ---
    if RR_avg2 > 0
        if abs(RR_avg1 - RR_avg2) > 0.2*RR_avg2
            THR_SIG = 0.5 * THR_SIG;
        end
    end
    ...
    if RR_avg1 > 0
        ...
        if (loc - last_peak) > round(1.66 * RR_avg1 * Fs)
            ...
            if search_end > search_start
                ...
                segment = energy_smooth(search_start:search_end);
                [missed_pk, idx] = max(segment);

                if missed_pk > THR_NOISE

                    true_loc = search_start + idx - 1;

                    detected_peaks(end+1) = true_loc;
                    last_peak = true_loc;

                    THR_SIG = 0.25*missed_pk + 0.75*THR_SIG;
                end
            end
        end
    end
...
for i = 1:length(detected_peaks)
    ...
    l = max(1, detected_peaks(i)-round(0.1*Fs));
    r = min(length(ecg), detected_peaks(i)+round(0.1*Fs));

    [~,idx] = max(ecg(l:r));
    refined(end+1) = l + idx - 1;
end

detected_peaks = unique(refined);

%% ================= FINAL RR FILTER =================
if length(detected_peaks) > 1
    RR = diff(detected_peaks)/Fs;
    valid = [true RR > 0.3 & RR < 1.5];
